...
data_join_2 <- 
  full_join(data_left, data_right_2, by = c("country" = "location", "year" = "date"))
```
...
# or
data_join_3 <- full_join(data_left, data_right_3, by = c("country"))
```
...
data_join_1952 <- data_join %>% 
  filter(year == 1952)

map_data_join_1952 <- full_join(data_join_1952, map_world, by = c("country" = "region"))

ggplot(map_data_join_1952) +
  aes(x = long, y = lat, group = group, fill = lifeExp) +
  geom_polygon()
```
